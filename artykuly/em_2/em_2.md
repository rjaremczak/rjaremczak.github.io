...
; ARG1 - adres bufora drugiego argumentu i wyniku
      LXI B, ARG0		; ładuj do pary B,C adres argumentu 0
LXI H, ARG1		; ładuj do pary H,L adres argumentu 1
	XRA A             ; zeruj akumulator i znacznik C
      MVI E, 8          ; ustaw licznik bajtów na 8 (64 bity)
NAST: 
LDAX B	      ; ładuj do A bieżący bajt argumentu 0
ADC M		      ; dodaj do A bieżący bajt argumentu 1
MOV M, A		; zapisz wynik do bieżącego bajtu argumentu 1
INX B		      ; zwiększ o 1 adres bajtu argumentu 0
INX H	            ; zwiększ o 1 adres bajtu argumentu 1
DCR E		      ; zmniejsz o 1 licznik bajtów
JNZ NAST		; jeśli nie wszystkie bajty, kontynuuj
10
10
4
...
Jest to kierunek myślenia zupełnie odwrotny od tego przyjętego przez firmę Intel. Pozwala uniknąć wielu pułapek specjalizacji, choć niekiedy kosztem większego nakładu pracy w procesie projektowania. Pierwszą cechą projektów wychodzących od generalizacji jest zazwyczaj prostota, co widać dobrze na Rysunku 3.
...
64 KiB (16-bitowy adres)
adresowanie portów
wspólne z adresowaniem pamięci
przerwania
niemaskowalne
maskowalne
tryby adresowania
...
; bufory muszą w całości mieścić się w zakresie adresów 0x00-0xFF
      LDX #0            ; zeruj rejestr indeksowy
      CLC               ; zeruj znacznik C
NAST: 
LDAA X, ARG0	; ładuj do A bieżący bajt argumentu 0
ADCA X, ARG1      ; dodaj do A bieżący bajt argumentu 1
STAA X, ARG1      ; zapisz wynik do bieżącego bajtu argumentu 1
INX		      ; zwiększ o 1 wartość rejestru indeksowego
CPX #8            ; sprawdź, czy przetworzono wszystkie bajty
BNE NAST		; jeśli nie, kontynuuj
3
2
...
16-bitowy wskaźnik stosu
rejestry
dwa zestawy po 6 rejestrów 8-bitowych
dwa 16-bitowe rejestry indeksowe
8-bitowy rejestr wektora przerwań
8-bitowy rejestr odświeżania pamięci DRAM
akumulator
dwa 8-bitowe akumulatory
znaczniki
...
adresowanie portów
wejście/wyjście: 256 x 8 bitów
nieudokumentowane adresowanie 16-bitowe
przerwania
niemaskowalne
maskowalne wektoryzowane (3 tryby pracy)
tryby adresowania
domyślne
natychmiastowe (8, 16 bitów)
rejestrowe
...
; ARG0 - adres bufora pierwszego argumentu
; ARG1 - adres bufora drugiego argumentu i wyniku
      LD BC, ARG0		; ładuj do pary B,C adres argumentu 0
LD HL, ARG1		; ładuj do pary H,L adres argumentu 1
	XOR A             ; zeruj akumulator i znacznik C
      LD E, 8          ; ustaw licznik bajtów na 8 (64 bity)
NAST: 
LD A, (BC)	      ; ładuj do A bieżący bajt argumentu 0
ADC A, (HL)	      ; dodaj do A bieżący bajt argumentu 1
LD (HL), A		; zapisz wynik do bieżącego bajtu argumentu 1
INC BC	      ; zwiększ o 1 adres bajtu argumentu 0
INC HL            ; zwiększ o 1 adres bajtu argumentu 1
DEC E		      ; zmniejsz o 1 licznik bajtów
JR NZ, NAST		; jeśli nie wszystkie bajty, kontynuuj
10
10
4
7
...
16 bitów
stos
16-bitowy wskaźnik stosu, bardziej znaczący bajt ma ustawioną na stałe wartość 0x01
rejestry
dwa 8-bitowe rejestry indeksowe
akumulator
8-bitowy akumulator
znaczniki
N – znak
...
względne
pośrednie
pośrednie indeksowane (addr) + Y
indeksowane pośrednie (addr + X)
architektura
wspólna przestrzeń adresowa dla programu i danych (von Neumanna)
...
; ARG1 - 8-bitowy adres bufora drugiego argumentu i wyniku
; bufory muszą w całości mieścić się w zakresie adresów 0x00-0xFF
      LDX #0            ; zeruj rejestr indeks bajtów
      LDY #8            ; ładuj licznik bajtów dla 64 bitów
      CLC               ; zeruj znacznik przeniesienia
NAST: 
LDA ARG0,X	      ; ładuj do A bieżący bajt argumentu 0
ADC ARG1,X        ; dodaj do A bieżący bajt argumentu 1
STA ARG1,X        ; zapisz wynik do bieżącego bajtu argumentu 1
INX		      ; zwiększ o 1 indeks bajtów
DEY               ; zmniejsz o 1 licznik bajtów
BNE NAST		; kontynuuj, jeśli nie wszystkie bajty dodano
2
2
2
